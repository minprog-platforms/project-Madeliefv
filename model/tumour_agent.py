...
"""
tumour_agent.py
* contains two agent classes for tumour model: tumour_model.py
    * Tumour_agent : contains three different tumour cells:
        * stem_cell
        * transit_amplifying
        * differentiated
    * Chemo_agent: class for chemo molecule agents
"""
...
class Tumour_agent(Agent):
    ...
    def choice_direction(self, agent_to_direct):
        ...
        # By 80% chance change either x or y to 0 -> for an equal distribution
...
class Chemo_agent(Agent):
    ...
    def move(self):
        ...
        new_position = random.choices([other_position, [(x_new, y_new)]],
                                      weights=[1 - self.model.vascularisation / 10, self.model.vascularisation / 10], k=1)[0]
    ...
    def worked_out(self):
        """Decides if the agent is worked out, based on age and cells killed and if so removes it"""
        if self.age > random.randint(10, 100) or self.cells_killed > random.randint(5, 10):
            self.model.grid._remove_agent(self.pos, self)
            self.model.schedule_chemo.remove(self)

    def step(self):
        """Performs step of chemo agent"""
        self.move()
        self.kill()
        self.worked_out()
        self.age += 1
